 ...
 */
public class BuyerTaskStep3 {

    public static final String TABLE_NAME = "buyer_task_step3";
     ...
     */
    public void create(BuyerTask3 buyerTask) {
        ...
        try {
            ...
            this.createTime = now;
            ss.getMapper(BuyerTaskStepMapper3.class).insert(this);

            /*
             *  更新任务状态
             */
            BuyerTask3 bt = BuyerTask3.instance(buyerTask.id).buyerId(buyerId).modifyTime(now);
            ...
            if (BuyerTaskStepType.ORDER_AND_PAY == type) {
                ...
                if (buyerTask.messageId != null) {
                    TaskOrderMessageMapper3 tomMapper = ss.getMapper(TaskOrderMessageMapper3.class);
     ...
     */
    public static List<BuyerTaskStep3> findByTaskId(long buyerTaskId) {
        List<BuyerTaskStep3> list = null;
        SqlSession ss = SessionFactory.getSqlSession();
        try {
            BuyerTaskStepMapper3 mapper = ss.getMapper(BuyerTaskStepMapper3.class);
            list = mapper.slelectByBuyerTaskId(buyerTaskId);
        } finally {
            ss.close();
        }
        
        for (BuyerTaskStep3 step : list) {
     ...
     */
    public static List<BuyerTaskStep3> findByTaskIdForSysRefund(long buyerTaskId) {
        List<BuyerTaskStep3> list = null;
        SqlSession ss = SessionFactory.getSqlSession();
        ...
        try {
            BuyerTaskStepMapper3 mapper = ss.getMapper(BuyerTaskStepMapper3.class);
            list = mapper.slelectByBuyerTaskId(buyerTaskId);
            BuyerTask3 buyerTask=BuyerTask3.findById(buyerTaskId);
            if(buyerTask!=null){
            	rest=Task3.isTaoBaoAndTmall(buyerTask.taskId);
            }
        } finally {
            ss.close();
        }
        ...
        Map<BuyerTaskStepType, BuyerTaskStep3> map = Maps.newLinkedHashMap();
        for (BuyerTaskStepType type : BuyerTaskStepType.values()) {
            ...
            map.put(type, BuyerTaskStep3.valueOf(type));
        }
     ...
     */
    public static BuyerTaskStep3 findBuyerLastStep(long buyerTaskId,long buyerId) {
        SqlSession ss = SessionFactory.getSqlSession();
        try {
            BuyerTaskStepMapper3 mapper = ss.getMapper(BuyerTaskStepMapper3.class);
    ...
    public static BuyerTaskStep3 newInstance(long buyerTaskId, long buyerId) {
        BuyerTaskStep3 sts = new BuyerTaskStep3();
        sts.buyerTaskId = buyerTaskId;
        sts.buyerId = buyerId;
        return sts;
    }
    
    public static BuyerTaskStep3 instance(long id) {
        BuyerTaskStep3 sts = new BuyerTaskStep3();
        sts.id = id;
        return sts;
    }
    
    public static BuyerTaskStep3 valueOf(BuyerTaskStepType type) {
        BuyerTaskStep3 sts = new BuyerTaskStep3();
        return sts.type(type);
    }
    
    public BuyerTaskStep3 type(BuyerTaskStepType type) {
        this.type = type;
        this.no = type.getOrder();
        return this;
    }
    
    public BuyerTaskStep3 content(BuyerTaskStepVo vo) {
        if (vo != null) {
            this.stepVo = vo;
    ...
    public BuyerTaskStep3 valid(boolean isValid) {
        this.isValid = isValid;
        return this;
    }
    
    public BuyerTaskStep3 createTime(Date time) {
        this.createTime = time;
        return this;
    }
    
    public BuyerTaskStep3 modifyTime(Date time) {
