 ...
 */
public class BuyerAccount3 {
    
    public static final String TABLE_NAME = "buyer_account3";
     ...
     */
    public static List<BuyerAccount3> findList(BuyerAccountSearchVo3 vo) {
        SqlSession ss = SessionFactory.getSqlSession();
        try{
        	BuyerAccountMapper3 mapper = ss.getMapper(BuyerAccountMapper3.class);
        	BuyerTaskMapper3 taskMapper = ss.getMapper(BuyerTaskMapper3.class);
        	List<BuyerAccount3> list = mapper.selectByUserIdAndPlatform(vo.userId, vo.platform);
        	
        	BuyerTaskSearchVo3 taskVo = new BuyerTaskSearchVo3();
        	taskVo.buyerId = vo.userId;
        	for(BuyerAccount3 ba: list){
        	    // 设置“审核不通过”的原因
     ...
     * TODO 获取用于接手任务的买号.
     */
    public static List<BuyerAccount3>  findForTakingTask(BuyerAccountSearchVo3 vo) {
        SqlSession ss = SessionFactory.getSqlSession();
        try{
            BuyerAccountMapper3 mapper = ss.getMapper(BuyerAccountMapper3.class);
            //TODO 过滤出符合条件的买号
	 ...
	 * 按页获取买号列表
	 */
	public static Page<BuyerAccount3> findForPage(BuyerAccountSearchVo3 vo) {
		SqlSession ss = SessionFactory.getSqlSession();
		try {
			BuyerAccountMapper3 baMapper = ss.getMapper(BuyerAccountMapper3.class);
			int count = baMapper.count(vo);
			if (count <= 0) {
				return Page.EMPTY;
			}
			Page<BuyerAccount3> page = Page.newInstance(vo.pageNo, vo.pageSize, count);
			page.items = baMapper.selectBySearch(vo);
			for (BuyerAccount3 ba : page.items) {
				if(StringUtils.isNotBlank(ba.memo)){
        	        ba.memos = JsonUtil.toList(ba.memo, Memo.class);
        	    }
				TaskStats3 taskStats = TaskStats3.findForBuyerUntilNow(ba.id);
				ba.weekTakenNum=taskStats.weekCount;
				ba.mouthTakenNum=taskStats.monthCount;
				ba.totalTakenNum=BuyerTask.findByBuyerAccount(ba.id);
			}
			return page;
		} finally {
			ss.close();
    ...
    public void updateStatus() {
        ...
        try {
            ...
            // 保存“审核不通过”时的理由
            if (this.status == ExamineStatus3.NOT_PASS) {
                ...
                // 取出原来memo信息
                BuyerAccount3 bacount = mapper.selectById(id);
     ...
     */
    public static BuyerAccount3 findById(Long id){
       	SqlSession ss = SessionFactory.getSqlSession();
        try{
        	BuyerAccountMapper3 mapper = ss.getMapper(BuyerAccountMapper3.class);
     ...
     */
    public static boolean validate(BuyerAccount3 account,User loginUser) {
        if (account == null) {
            return false;
        }
        return account.status == ExamineStatus3.EXAMINED && account.belongTo(loginUser);
    }
