...
//	public Date lastBatchPublishNo;
    ...
    public void save() {
        ...
        try {
			...
			TaskOrderMessage tom = TaskOrderMessage.newInstance(this.id);
			tom.usedNum = 0;
			...
			System.out.println("任务总单数-------1"+totalOrderNum);
			System.out.println("任务总单数-------1"+totalOrderNum);
			System.out.println("任务总单数-------1"+totalOrderNum);
			System.out.println("任务总单数-------1"+totalOrderNum);
			System.out.println("任务总单数-------1"+totalOrderNum);
	 ...
	 */
	public static String getItemIdByPlatform(Platform platform, String url) {
		String itemId = "";
		switch (platform) {
		case TAOBAO:
     ...
     * 上传直通车图片到七牛
     */
    private void uploadAndSetSubwayItemPic(TaskMapper mapper,boolean isNew) throws IOException {
        if (this.type != TaskType.SUBWAY) {
            return;
        }
    ...
    public void calculateAndSetTaskPledgeAndIngot() {
        ...
        long mobileOrderIngot = this.baseOrderIngot + (long) (BizConstants.TASK_MOBILE_INGOT * 100);
...
//        serviceFee += countExtraSearchPlan() * (BizConstants.TASK_SEARCH_PLAN_INGOT * 100);
    ...
    private void deducteAndApply(SqlSession ss) {
        ...
        Task t = Task.instance(this.id).setSellerId(this.sellerId).setStatus(TaskStatus.WAIT_EXAMINE);
        t.isPaid = true;
        t.modifyTime = now;
     ...
     */
    public static List<Task> findlockedPledgeTask(long sellerId) {
        SqlSession ss = SessionFactory.getSqlSession();
        try {
            TaskMapper mapper = ss.getMapper(TaskMapper.class);
     ...
     */
    public static Task findById(String fields, long id) {
        SqlSession ss = SessionFactory.getSqlSession();
        try {
            TaskMapper mapper = ss.getMapper(TaskMapper.class);
	 ...
	 */
	@NotNull
	public static Page<Task> findBySearchVo(TaskSearchVo vo) {
		...
		try {
			...
			Page<Task> page = Page.newInstance(vo.pageNo, vo.pageSize, count);
			List<Task> tasks = mapper.selectList(vo);
			for(Task task: tasks){
	 ...
	 */
	@NotNull
	public static Page<SellerTaskVo> findVoByPage(TaskSearchVo svo) {
		SqlSession ss = SessionFactory.getSqlSession();
		try {
			TaskMapper mapper = ss.getMapper(TaskMapper.class);
			...
			Page<SellerTaskVo> page = Page.newInstance(svo.pageNo, svo.pageSize, totalCount);
			List<SellerTaskVo> vos =  mapper.selectVoListForSeller(svo);
			
			// 若获取条件有状态审核不通过，给不通过原因设值
            for (SellerTaskVo vo : vos) {
                ...
                if (svo.status == TaskStatus.NOT_PASS && MixHelper.isNotEmpty(vo.examineLogs)) {
                    vo.examineLog = SellerTaskVo.getLastLog(vo);
                }
                // 返回任务状态的标题文字到前端
	 ...
	 */
	@NotNull
	public static Page<SellerTaskVo> findProcessTaskByPage(TaskSearchVo svo) {
		SqlSession ss = SessionFactory.getSqlSession();
		try {
			TaskMapper mapper = ss.getMapper(TaskMapper.class);
			...
			Page<SellerTaskVo> page = Page.newInstance(svo.pageNo, svo.pageSize, totalCount);
			List<SellerTaskVo> vos =  mapper.selectProcessTaskSeller(svo);
	 ...
	 */
	@NotNull
	public static Page<Task> findByPage(TaskSearchVo vo) {
	    ...
	    try {
	        ...
	        Page<Task> page = Page.newInstance(vo.pageNo, vo.pageSize, totalCount);
	 ...
	 */
	public static Page<SellerTaskVo> findExamineVoByPage(TaskSearchVo vo) {
		SqlSession ss = SessionFactory.getSqlSession();
		try {
			TaskMapper mapper = ss.getMapper(TaskMapper.class);
			...
			Page<SellerTaskVo> page = Page.newInstance(vo.pageNo, vo.pageSize, totalCount);
			page.items = mapper.selectVoForStatus(vo);
			for (SellerTaskVo v : page.items) {
     ...
     */
    public static Multimap<String, TaskCountVo> findWaitingTaskCountInfo(long sellerId) {
        List<TaskCountVo> list = Lists.newArrayList();
        SqlSession ss = SessionFactory.getSqlSession();
        try {
            TaskMapper mapper = ss.getMapper(TaskMapper.class);
     ...
     */
    public static Task findTodoTask(long sellerId) {
        SqlSession ss = SessionFactory.getSqlSession();
        try {
            TaskMapper mapper = ss.getMapper(TaskMapper.class);
     ...
     */
	public static long computeFeePerOrder(long itemPrice,Platform platform) {
        // 转换金额单位：从分到元
     ...
     */
    public static float computeCommission(float price,Platform platform) {
		...
		try {
			...
			if (this.status == TaskStatus.PUBLISHED || this.status == TaskStatus.WAIT_PUBLISH) {
				...
				if (CollectionUtils.isNotEmpty(searchPlan)) {
					...
					if (task.platform == Platform.TAOBAO || task.platform == Platform.TMALL || task.platform == Platform.JD)
						for (TaskItemSearchPlan plan : searchPlan) {
							...
							if (plan.flowNum > 0) {
								...
								TrafficRecordVo trafficRecord = TrafficRecordVo.init(task, plan.flowNum, plan.word);
            ...
            if (this.status == TaskStatus.PUBLISHED) {
                Task.updateTaskCount(Sign.PLUS, task.totalOrderNum);
            }
		} finally {
			ss.close();
		}
    ...
    public static void cancledAndRefund(long id) {
        // 获取任务退款金额
        CancelTaskVo cancledInfo = findCancelInfo(id);
        // 若任务全部被接单
        ...
        try {
            if (cancledInfo != null) {
                ...
                // 更新“任务列表(X)”中的任务数量
                Task.updateTaskCount(Sign.MINUS, cancledInfo.cancledNum);
                // 更新user缓存
                User.findByIdWichCache(seller.id).pledge(record.balance).ingot(ingotRecord.balance).updateCache();
            }
            // 更新任务状态为取消
            Task ta = Task.instance(id).setSellerId(seller.id).setStatus(TaskStatus.CANCLED).modifyTime(now);
            ss.getMapper(TaskMapper.class).updateById(ta);
            ss.commit();
        } catch (Exception e) {
            ss.rollback();
            log.error(e.getMessage(), e);
	 ...
	 */
	public static CancelTaskVo findCancelInfo(long id) {
		SqlSession ss = SessionFactory.getSqlSession();
		try {
			// 取出已接单数量
			TaskMapper mapper = ss.getMapper(TaskMapper.class);
            Task t = mapper.selectById(id);
            ...
            if (t.pcTakenCount <= 0 && t.mobileTakenCount <= 0) {
                CancelTaskVo vo =  CancelTaskVo.newInstance(t.totalOrderNum, t.totalPledge, t.totalIngot);
	 ...
	 */
	   ...
	   */
	  public static List<Task> findSellerPublishCount(String publishTime) {
          SqlSession ss = SessionFactory.getSqlSession();
          try {
              TaskMapper mapper = ss.getMapper(TaskMapper.class);
	 ...
	 */
	public static Task findByItemId(String itemId) {
		SqlSession ss = SessionFactory.getSqlSession();
		try {
			TaskMapper mapper = ss.getMapper(TaskMapper.class);
	 ...
	 */
	   public static void updateTaskExpressType() {
	        ...
	        try {
	            TaskMapper mapper = ss.getMapper(TaskMapper.class);
	            List<Task> tasks=mapper.findTaskExpressType();
    ...
    public static Task instance(long id) {
        Task task = new Task();
        task.id = id;
        return task;
    }
    
    public static Task instanceForTest(long id) {
	...
	public Task setSellerId(long sellerId) {
    ...
    public Task setPublishTimerInterval(int publishTimerInterval) {
        this.publishTimerInterval = publishTimerInterval;
        return this;
    }

    public Task setLastBatchPublishTime(Date lastBatchPublishTime) {
        this.lastBatchPublishTime = lastBatchPublishTime;
        return this;
    }
    
    public Task speedTaskIngot(int speedTaskIngot) {
        this.speedTaskIngot = speedTaskIngot;
        return this;
    }
    
    public Task extraRewardIngot(int extraRewardIngot) {
        this.extraRewardIngot = extraRewardIngot;
        return this;
    }

    public Task modifyTime(Date modifyTime){
        this.modifyTime = modifyTime;
        return this;
    }
    
    public Task shopId(long shopId){
        this.shopId = shopId;
        return this;
    }
    
    public Task shopName(String shopName) {
        this.shopName = shopName;
        return this;
    }

    public Task sellerNick(String sellerNick) {
        this.sellerNick = sellerNick;
        return this;
    }
    
    public Task platform(Platform p) {
        this.platform = p;
        return this;
    }
    
    public Task orderNum(Device d, int orderNum) {
        Validate.notNull(d);
        if (d == Device.PC) {
    ...
    public Task itemUrl(String itemUrl) {
    ...
    @NotNull
	public static List<Task> findALLList(TaskSearchVo vo) {
		...
		try {
			TaskMapper mapper = ss.getMapper(TaskMapper.class);
			List<Task> tasks = mapper.selectAllList(vo);
