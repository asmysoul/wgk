 ...
 */
public class BuyerTask2 {
	
	private static final Logger log = LoggerFactory.getLogger(BuyerTask2.class);

	public static final String TABLE_NAME = "buyer_task2";
	public Long id;
	 ...
	 * @since v0.1
	 */
	public BuyerTask2(OrderExpress order) {
	 ...
	 */
	public void create() {
		Validate.notNull(this.device, "BuyerTask2.device must not be null");
		...
		try {
			...
			Task2 task = taskMapper.selectFieldsById("pc_order_num,mobile_order_num,pc_taken_count,mobile_taken_count,"
					+ "publish_timer_value,publish_timer_interval,last_batch_publish_time," + "total_order_num", this.taskId);

			// 累加已接手任务数
			Task2 taskToSave = Task2.instance(this.taskId).modifyTime(DateTime.now().toDate());
			...
			// 再次检查已被领取的任务数量
			TaskSearchVo2 svo = TaskSearchVo2.newInstance().taskId(taskId).statusExclude(TaskStatus.CANCLED);
			...
			if (task.checkIsTimerTask() && task.checkIsCurrentGroupTasksTakenOver()) {
				TaskKeeper2.log.info("Put Task={} into pool, after buyer taken buyerTask={}", this.taskId, this.id);
			...
			// 更新“任务列表(X)”中的任务数量
			Task2.updateTaskCount(Sign.MINUS, 1);
    ...
    private static boolean _checkBuyerTaskExist(long buyerAccountId, long taskId, BuyerTaskMapper2 btMapper) {
        TaskSearchVo2 vo = TaskSearchVo2.newInstance().status(TaskStatus.CANCLED).taskId(taskId);
	 ...
	 * @param buyerId
	 */
	public static Multimap<String, TaskCountVo2> findWaitingTaskCountInfo(long buyerId) {
		SqlSession ss = SessionFactory.getSqlSession();
		try {
			BuyerTaskMapper2 mapper = ss.getMapper(BuyerTaskMapper2.class);
			Multimap<String, TaskCountVo2> map = ArrayListMultimap.create();
			// 统计
			for (TaskCountVo2 taskCountVo : mapper.countWaitingTasksByBuyerId(buyerId)) {
			...
			List<TaskCountVo2> vos = (List<TaskCountVo2>) map.get(TaskStatus.WAIT_PAY.name());
			if (MixHelper.isEmpty(vos)) {
				return map;
			}

			// WAIT_PAY, RECIEVED 用一个key标记（recieved）
			TaskCountVo2 weixin = new TaskCountVo2(Platform2.WEIXIN, TaskStatus.WAIT_PAY);
			TaskCountVo2 weibo = new TaskCountVo2(Platform2.WEIBO, TaskStatus.WAIT_PAY);
			List<TaskCountVo2> tvos = Lists.newArrayList();
			for (TaskCountVo2 vo : vos) {
				if (vo.platform == Platform2.WEIXIN) {
					weixin.count = weixin.count + vo.count;
				} else {
					weibo.count = weibo.count + vo.count;
				}
			}
			tvos.add(weixin);
			tvos.add(weibo);
	 ...
	 */
	public static Page<BuyerTaskVo2> findListByPage(TaskSearchVo2 vo) {
		SqlSession ss = SessionFactory.getSqlSession();
		try {
			BuyerTaskMapper2 mapper = ss.getMapper(BuyerTaskMapper2.class);
			int totalCount;
			if (vo.module == SearchModule2.SELLER_REFUND) {
				totalCount = mapper.count(vo);
			} else if (vo.module != null) {
				// 提现时仅查询“已完成”且“未提现”的任务记录
				if (vo.module == SearchModule2.SYS_REFUND_WITHDRAW) {
			...
			Page<BuyerTaskVo2> page = Page.newInstance(vo.pageNo, vo.pageNo, totalCount);
			List<BuyerTaskVo2> vos = null;
			// 获取商家退款所需的数据
			if (vo.module == SearchModule2.SELLER_REFUND) {
	 ...
	 */
	public static Page<OrderExpress> findOrdersByPage(TaskSearchVo2 vo) {
		SqlSession ss = SessionFactory.getSqlSession();
		try {
			BuyerTaskMapper2 btMapper = ss.getMapper(BuyerTaskMapper2.class);
	 ...
	 */
	public static List<BuyerTaskVo2> findListForConfirm(TaskSearchVo2 vo) {
	 ...
	 */
	public static List<OrderExpress> findOrders(TaskSearchVo2 vo) {
		SqlSession ss = SessionFactory.getSqlSession();
		try {
			BuyerTaskMapper2 btMapper = ss.getMapper(BuyerTaskMapper2.class);
	 ...
	 */
	public static Page<BuyerTask2> findByPageForSellerTask(TaskSearchVo2 vo) {
		...
		try {
			...
			Page<BuyerTask2> page = Page.newInstance(vo.pageNo, vo.pageSize, totalCount);

			// 赋值statusTitle字段 方便显示状态信息
			List<BuyerTask2> list = mapper.selectListForTaskDetail(vo);
			for (BuyerTask2 bt : list) {
	 ...
	 */
	public static Integer modify(BuyerTask2 bt) {
	 ...
	 */
    public static Integer modifyYDKD(BuyerTask2 bt) {
        Validate.notNull(bt.id);
        SqlSession ss = SessionFactory.getSqlSession();
        try {
            BuyerTaskMapper2 btMapper = ss.getMapper(BuyerTaskMapper2.class);
	...
	public void confirmGoodsAndRate(BuyerTaskStepVo stepVo) {
		...
		try {
			// 保存任务进度
			BuyerTaskStep2 taskStep = new BuyerTaskStep2();
			...
			BuyerTask2 buyerTask = BuyerTask2.instance(this.id).status(TaskStatus.WAIT_REFUND).modifyTime(now);
			Task2 task = ss.getMapper(TaskMapper2.class).selectFieldsById("sys_refund", this.taskId);
			if (task.sysRefund) {
	...
	public void confirmSellerRefund2() {
		System.out.println("++++++++++++++++");
		SqlSession ss = SessionFactory.getSqlSessionWithoutAutoCommit();
		try {
			Date now = DateTime.now().toDate();
	...
	private User processRefund(SqlSession ss, Date now, long paidFee) {
		 ...
		 */
		BuyerTaskMapper2 mapper = ss.getMapper(BuyerTaskMapper2.class);
		BuyerTask2 buyerTask = BuyerTask2.instance(this.id).buyerId(this.buyerId).status(TaskStatus.FINISHED);
		 ...
		 */
		TaskMapper2 taskMapper = ss.getMapper(TaskMapper2.class);
		Task2 t = taskMapper.selectFieldsById(
		...
		if (BooleanUtils.isTrue(t.sysRefund)) {
			...
			SellerPledgeRecord pledgeRecord = SellerPledgeRecord.newInstance(t.sellerId, lastPledge).taskId(taskId)
					.action(PledgeAction.UNLOCK, 0).createTime(now).memo("买手核实退款，解冻押金，推广编号：[" + t.id + "-" + buyerTask.id + "]");
			...
			taskStep.buyerId = this.buyerId;
			taskStep.buyerTaskId = this.id;
			BuyerTaskStepMapper2 btMapper = ss.getMapper(BuyerTaskStepMapper2.class);
			taskStep.no = taskStep.type.getOrder();
			taskStep.createTime = now;
		...
		Task2 task = Task2.instance(taskId);
	 ...
	 */
	public static BuyerTaskVo2 findVoById(long id, long buyerId) {
		SqlSession ss = SessionFactory.getSqlSession();
		try {
			BuyerTaskMapper2 mapper = ss.getMapper(BuyerTaskMapper2.class);
	 ...
	 */
	public static BuyerTaskVo2 findForPerforming(long id, long buyerId) {
		SqlSession ss = SessionFactory.getSqlSession();
		try {
			BuyerTaskMapper2 mapper = ss.getMapper(BuyerTaskMapper2.class);
			BuyerTaskVo2 vo = mapper.selectVoByIdForPerform(id, buyerId);
			System.out.println("评论：--------------"+vo.goodCommentWords);
			System.out.println("评论：--------------"+vo.goodCommentWords);
			System.out.println("评论：--------------"+vo.goodCommentWords);
			System.out.println("评论：--------------"+vo.goodCommentWords);
			System.out.println("评论：--------------"+vo.goodCommentWords);
	 ...
	 */
	public static BuyerTaskVo2 findDetail(long id) {
		SqlSession ss = SessionFactory.getSqlSession();
		try {
			BuyerTaskMapper2 mapper = ss.getMapper(BuyerTaskMapper2.class);
			BuyerTaskVo2 buyerTaskVo = mapper.selectByIdForViewDetail(id);
	 ...
	 */
	public static BuyerTask2 findById(long id) {
		SqlSession ss = SessionFactory.getSqlSession();
		try {
			BuyerTaskMapper2 mapper = ss.getMapper(BuyerTaskMapper2.class);
	 ...
	 */
    public static BuyerTask2  findByOrderId(String orderId) {
        SqlSession ss = SessionFactory.getSqlSession();
        try {
            BuyerTaskMapper2 mapper = ss.getMapper(BuyerTaskMapper2.class);
	 ...
	 */
	public static BuyerTaskVo2 findByIdForTaskType(long id) {
		SqlSession ss = SessionFactory.getSqlSession();
		try {
			BuyerTaskMapper2 mapper = ss.getMapper(BuyerTaskMapper2.class);
	...
	public static Integer getByType(long id) {
		BuyerTaskVo2 vo = findByIdForTaskType(id);
	 ...
	 */
	public static BuyerTask2 findTodoTask(long buyerId) {
		SqlSession ss = SessionFactory.getSqlSession();
		try {
			BuyerTaskMapper2 mapper = ss.getMapper(BuyerTaskMapper2.class);
			BuyerTask2 buyerTask = mapper.selectLastWaitPayAndStepByBuyerId(buyerId);
			if (buyerTask == null) {
				return null;
			}

			// 默认是第一步
			if(Task2.isTaoBaoAndTmall(buyerTask.taskId)){
	...
	public static boolean isParentTaskFinished(long taskId) {
		...
		try {
			BuyerTaskMapper2 mapper = ss.getMapper(BuyerTaskMapper2.class);
			TaskCountVo2 vo = mapper.countForTaskFinish(taskId);
	 ...
	 */
	public BuyerTask2 simpleCreate() {
		SqlSession ss = SessionFactory.getSqlSession();
		try {
			BuyerTaskMapper2 mapper = ss.getMapper(BuyerTaskMapper2.class);
			this.modifyTime = this.takeTime;
	 ...
	 */
	public static List<BuyerTask2> findList(TaskSearchVo2 vo) {
		SqlSession ss = SessionFactory.getSqlSession();
		try {
			BuyerTaskMapper2 mapper = ss.getMapper(BuyerTaskMapper2.class);
	 ...
	 * @created 2016-8-3
	 */
	public static Integer findCountByPlatform(Platform2 platform) {
		SqlSession ss = SessionFactory.getSqlSession();
		try {
	 ...
	 */
	public static Integer findPcCountByPlatform(Platform2 platform, TaskType2 taskType) {
		SqlSession ss = SessionFactory.getSqlSession();
		try {
			TaskMapper2 mapper = ss.getMapper(TaskMapper2.class);
			if(taskType == null) {
	 ...
	 * @created 2015-2-27 上午11:13:05
	 */
	public static Integer findPadCountByPlatform(Platform2 platform, TaskType2 taskType) {
		SqlSession ss = SessionFactory.getSqlSession();
		try {
			TaskMapper2 mapper = ss.getMapper(TaskMapper2.class);
			if(taskType == null) {
	...
	public void sendGood() {
		...
		try {
			...
			BuyerTask2 bta = btMapper.selectById(this.id);
			taskStep.buyerId = bta.buyerId;
			taskStep.buyerTaskId = this.id;
			BuyerTaskStepMapper2 btsMapper = ss.getMapper(BuyerTaskStepMapper2.class);
			taskStep.no = taskStep.type.getOrder();
	...
	public void cancel() {
		...
		try {
			...
			// 更新“任务推广(X)”中的任务数量
			Task2.updateTaskCount(Sign.PLUS, 1);
			...
			ss.rollback();
			log.error(e.getMessage(), e);
			throw new RuntimeException("撤销推广操作失败！");
		} finally {
			ss.close();
		}
	}
	...
	public void cancelStep(SqlSession ss) {
		BuyerTaskMapper2 mapper = ss.getMapper(BuyerTaskMapper2.class);
		BuyerTask2 bt = mapper.selectById(this.id);
		...
		// 更新主任务接手数量
		TaskMapper2 tMapper = ss.getMapper(TaskMapper2.class);
		Task2 task = tMapper.selectFieldsById("*", bt.taskId);
		bt.updateTaskTakenCount(tMapper, task, Sign.MINUS);

		// 如果主任务为已取消状态，将撤销子任务的费用退还给卖家。
		if (task.status == task.status.CANCLED) {
			BuyerTaskProcessor2.refundCancledBuyerTask(ss, task, bt);
		}
	}

	// 根据任务终端对父任务的“已接手数”进行增减
	private Task2 updateTaskTakenCount(TaskMapper2 taskMapper, Task2 t, Sign sign) {
	...
	public static BuyerTask2 newInstance() {
		BuyerTask2 bt = new BuyerTask2();
		return bt;
	}

	public static BuyerTask2 instance(Long id) {
		BuyerTask2 bt = new BuyerTask2();
		bt.id = id;
		return bt;
	}

	public BuyerTask2 buyerId(Long buyerId) {
		this.buyerId = buyerId;
		return this;
	}

	public BuyerTask2 buyerAccountId(Long buyerAccountId) {
		this.buyerAccountId = buyerAccountId;
		return this;
	}

	public BuyerTask2 status(TaskStatus status) {
		this.status = status;
		return this;
	}

	public BuyerTask2 modifyTime(Date modifyTime) {
		this.modifyTime = modifyTime;
		return this;
	}

	public BuyerTask2 takeTime(Date takeTime) {
		this.takeTime = takeTime;
		return this;
	}

	public BuyerTask2 taskId(Long taskId) {
		this.taskId = taskId;
		return this;
	}

	public BuyerTask2 paidFee(Long paidFee) {
		this.paidFee = paidFee;
		return this;
	}

	public BuyerTask2 withdrawSn(Long userWithdrawRecordSn) {
	...
	public static int findSysRefundCount(User user) {
		...
		try {
			...
			TaskSearchVo2 vo = TaskSearchVo2.newInstance();
	 ...
	 */
    public static Page<BuyerTaskVo2> findByPageForAdmin(TaskSearchVo2 vo) {
		SqlSession ss = SessionFactory.getSqlSession();
		try {
			BuyerTaskMapper2 mapper = ss.getMapper(BuyerTaskMapper2.class);
			...
			List<BuyerTaskVo2> vos = mapper.selectListForAdmin(vo);
			// 赋值taskIdStr
			for (BuyerTaskVo2 btVo : vos) {
				btVo.value();
				if (btVo.itemPicUrl != null && btVo.itemPicUrl.startsWith("{")) {
			...
			Page<BuyerTaskVo2> page = Page.newInstance(vo.pageNo, vo.pageSize, totalCount);
			page.items = vos;
			return page;
		} finally {
			ss.close();
	...
	//获取某特定状态下的任务数
	public static int count(TaskSearchVo2 vo) {
		SqlSession ss = SessionFactory.getSqlSession();
		try {
			BuyerTaskMapper2 btMapper = ss.getMapper(BuyerTaskMapper2.class);
	 ...
	 * @return
	 */
	public static List<OrderExpress> waitPrintOrders(TaskSearchVo2 vo) {
		SqlSession ss = SessionFactory.getSqlSession();
		try {
			BuyerTaskMapper2 btMapper = ss.getMapper(BuyerTaskMapper2.class);
	 ...
	 * @return
	 */
	public static Integer updateExpressNoById(BuyerTask2 bt) {
		Validate.notNull(bt.id);
	 ...
	 */
	public static Page<SellerBalanceVo2> findBuyerTaskBalance(SellerBalanceVo2 vo) {
		SqlSession ss = SessionFactory.getSqlSession();
		try {
			BuyerTaskMapper2 btMapper = ss.getMapper(BuyerTaskMapper2.class);
			...
			Page<SellerBalanceVo2> page =Page.newInstance(vo.pageNo, vo.pageSize, totalCount);
	 ...
	 * @created 2015年3月2日 上午10:36:05
	 */
	public static List<SellerBalanceVo2> exprotSellerTaskBalance(SellerBalanceVo2 vo) {
		SqlSession ss = SessionFactory.getSqlSession();
		try {
			BuyerTaskMapper2 btMapper = ss.getMapper(BuyerTaskMapper2.class);
	 ...
	 * @created 2015年3月2日 上午10:36:05
	 */
	public static List<SellerBalanceVo2> exprotBuyerTaskBalance(SellerBalanceVo2 vo) {
		SqlSession ss = SessionFactory.getSqlSession();
		try {
			BuyerTaskMapper2 btMapper = ss.getMapper(BuyerTaskMapper2.class);
	 ...
	 * @created 2015年3月2日 上午10:36:05
	 */
	public static Page<SellerBalanceVo2> findSellerTaskBalance(SellerBalanceVo2 vo) {
		SqlSession ss = SessionFactory.getSqlSession();
		try {
			BuyerTaskMapper2 btMapper = ss.getMapper(BuyerTaskMapper2.class);
     ...
     */
    public static Page<FaBaoGuoVo> findFaBaoGuoOrders(TaskSearchVo2 vo) {
        SqlSession ss = SessionFactory.getSqlSession();
     ...
     */
    public static List<FaBaoGuoVo> exportFaBaoGuoOrders(TaskSearchVo2 vo) {
        SqlSession ss = SessionFactory.getSqlSession();
        try {
            BuyerTaskMapper2 btMapper = ss.getMapper(BuyerTaskMapper2.class);
