...
public class Task2 {
	...
	public static final String TABLE_NAME = "task2";
	...
	public List<TaskItemSearchPlan> searchPlans;
...
//	public Date lastBatchPublishNo;
	...
	public String toString() {
		return "Task2 [id=" + id + ", platform=" + platform + ", type=" + type
				+ ", shopId=" + shopId + ", shopName=" + shopName + ", itemId="
				+ itemId + ", itemTitle=" + itemTitle + ", itemUrl=" + itemUrl
				+ ", itemProps=" + itemProps + ", itemPrice=" + itemPrice
				+ ", itemDisplayPrice=" + itemDisplayPrice + ", itemBuyNum="
				+ itemBuyNum + ", itemPicUrl=" + itemPicUrl + ", itemPic="
				+ itemPic + ", itemSubwayPicUrl=" + itemSubwayPicUrl
				+ ", itemSubwayPic=" + itemSubwayPic + ", itemSearchMinPrice="
				+ itemSearchMinPrice + ", itemSearchMaxPrice="
				+ itemSearchMaxPrice + ", itemSearchLocation="
				+ itemSearchLocation + ", searchPlans=" + searchPlans
				...
				+ ", experience=" + experience + ", extraRewardIngot="
				+ extraRewardIngot + ", baseOrderIngot=" + baseOrderIngot
				+ ", vasIngot=" + vasIngot + ", totalIngot=" + totalIngot
				+ ", pledge=" + pledge + ", totalPledge=" + totalPledge
				+ ", sysRefund=" + sysRefund + ", delaySpan=" + delaySpan
				+ ", speedTaskIngot=" + speedTaskIngot + ", speedExamine="
				+ speedExamine + ", AddFen=" + AddFen
				...
				+ ", taskRequest=" + taskRequest + ", sellerAdminName="
				+ sellerAdminName + ", goodCommentImg=" + goodCommentImg
    ...
    public void save() {
        ...
        try {
             ...
             * 处理关键词方案
             */
            TaskItemSearchPlanMapper planMapper = ss.getMapper(TaskItemSearchPlanMapper.class);
            TaskOrderMessageMapper messageMapper = ss.getMapper(TaskOrderMessageMapper.class);
			...
			this.totalOrderNum = this.mobileOrderNum;
			System.out.println("任务总单数-------"+totalOrderNum+"-------加粉"+this.AddFen);
			System.out.println("任务总单数-------"+totalOrderNum+"-------加粉"+this.AddFen);
	 ...
	 */
	public static String getItemIdByPlatform(Platform2 platform2, String url) {
		String itemId = "";
		switch (platform2) {
		case WEIXIN:
		...
		case WEIBO:
			itemId = UrlUtil.getParam("id", url);
//			itemId = StringUtils.substring(url, url.lastIndexOf("/") + 1, url.lastIndexOf("."));
			break;
		case OTHER:
			/*if (StringUtils.contains(url, "?")) {
				itemId = StringUtils.substring(url, url.lastIndexOf("/") + 1, url.indexOf("?"));
			} else {
				itemId = StringUtils.substring(url, url.lastIndexOf("/") + 1);
			}*/
			itemId = UrlUtil.getParam("id", url);
			break;
		default:
    ...
    private void uploadAndSetSubwayItemPic(TaskMapper2 mapper,boolean isNew) throws IOException {
        // 修改旧任务时检查是否有改动
        if (!isNew) {
      ...
      System.out.println("itemSubwayPic------------------"+this.itemSubwayPic);
      System.out.println("itemSubwayPic------------------"+this.itemSubwayPic);
      System.out.println("itemSubwayPic------------------"+this.itemSubwayPic);
      System.out.println("itemSubwayPic------------------"+this.itemSubwayPic);
      System.out.println("itemSubwayPic------------------"+this.itemSubwayPic);
    ...
    public void calculateAndSetTaskPledgeAndIngot() {
      ...
      /*  this.pledge = calculateItemPledge();
        ...
        if (BooleanUtils.isFalse(this.isFreeShipping)) {
            this.pledge += (BizConstants.TASK_EXPRESS_PLEDGE * 100);     // 每单任务非包邮商品运费押金：10元
        }
        // 押金总计
        this.totalPledge = this.pledge * (this.pcOrderNum + this.mobileOrderNum);
*/
        
    	this.totalPledge = (long) 0;
    	
        /*
         * 刷单服务费用（PC与mobile订单需要分开计算）
         */
        // 按照特定公式计算出的每单刷单基础费用
        this.baseOrderIngot = computeFeePerOrder(1,this.platform);
        
        System.out.println("按照特定公式计算出的每单刷单基础费用"+baseOrderIngot);
        System.out.println("按照特定公式计算出的每单刷单基础费用"+baseOrderIngot);
   
        
//        this.baseOrderIngot = computeFeePerOrder(this.itemPrice * this.itemBuyNum,this.platform);
        // /** 移动端每单任务费用：1元
        long mobileOrderIngot = this.baseOrderIngot + (long) (BizConstants.TASK_MOBILE_INGOT2 * 100);
       // long serviceFee = this.baseOrderIngot * this.pcOrderNum + mobileOrderIngot * this.mobileOrderNum;
        
        long serviceFee = mobileOrderIngot * this.mobileOrderNum;
        
        /*// 快递费
        serviceFee += (this.expressIngot * totalOrderNum);*/
        // 多关键词搜索(不再收费)
//        serviceFee += countExtraSearchPlan() * (BizConstants.TASK_SEARCH_PLAN_INGOT * 100);
        ...
        if (BooleanUtils.isTrue(this.speedExamine)) {
            this.vasIngot += (BizConstants.TASK_SPEED_EXAMINE_INGOT2 * 100);
        }
        // 加粉
        if (BooleanUtils.isTrue(this.AddFen)) {
            this.vasIngot += (BizConstants.TASK_ADD_FEN_INGOT2 * 100)* totalOrderNum;
        }
        ...
        if(StringUtils.isNotEmpty(this.goodCommentWords)){
        	System.out.println("朋友圈评论");
         	System.out.println("朋友圈评论");
         	System.out.println("朋友圈评论");
         	System.out.println("朋友圈评论");
            this.vasIngot += (BizConstants.TASK_GOOD_COMM_KWD_INGOT * 100)* totalOrderNum;
        }
        if(StringUtils.isNotEmpty(this.goodCommentImg)){
            this.vasIngot += (BizConstants.TASK_GOOD_COMM_PIC_INGOT * 100);
        }
        
        System.out.println("朋友圈评论服务"+goodCommentWords);
        System.out.println("朋友圈评论服务"+goodCommentWords);
        System.out.println("朋友圈评论服务"+goodCommentWords);
        System.out.println("朋友圈评论服务"+goodCommentWords);
        System.out.println("朋友圈评论服务"+goodCommentWords);
        System.out.println("加赏佣金服务------------------"+this.extraRewardIngot);
        System.out.println("加赏佣金服务------------------"+this.extraRewardIngot);
        System.out.println("加赏佣金服务------------------"+this.extraRewardIngot);
        System.out.println("加赏佣金服务------------------"+this.extraRewardIngot);
        System.out.println("加赏佣金服务------------------"+this.extraRewardIngot);
        ...
        this.totalIngot = serviceFee + this.vasIngot;
        System.out.println("增值服务费用"+vasIngot);
        System.out.println("增值服务费用"+vasIngot);
        System.out.println("增值服务费用"+vasIngot);
        System.out.println("增值服务费用"+vasIngot);
        System.out.println("增值服务费用"+vasIngot);
        ...
        System.out.println("增值服务费用"+vasIngot);
        System.out.println("=====================================================");
        System.out.println("总费用：刷单费用（服务费+快递费） + 增值服务"+totalIngot);
        System.out.println("总费用：刷单费用（服务费+快递费） + 增值服务"+totalIngot);
        System.out.println("总费用：刷单费用（服务费+快递费） + 增值服务"+totalIngot);
        System.out.println("总费用：刷单费用（服务费+快递费） + 增值服务"+totalIngot);
        System.out.println("总费用：刷单费用（服务费+快递费） + 增值服务"+totalIngot);
    ...
    public void pay(long payFee, boolean useIngot, boolean usePledge) {
        ...
        try {
            ...
            if (CollectionUtils.isNotEmpty(this.searchPlans)) {
				long totalFlow = 0;
				for (TaskItemSearchPlan plan : this.searchPlans) {
					// 跳过关键词为空的记录
					if (plan == null || StringUtils.isBlank(plan.word)) {
						continue;
					}
    ...
    private void deducteAndApply(SqlSession ss) {
        ...
        lastPledge = SellerPledgeRecord.newInstance(this.sellerId, lastPledge).action(action,0)
        ...
        // 更新任务状态
        TaskMapper2 taskMapper = ss.getMapper(TaskMapper2.class);
        Task2 t = Task2.instance(this.id).setSellerId(this.sellerId).setStatus(TaskStatus.WAIT_EXAMINE);
        t.isPaid = true;
        t.modifyTime = now;
     ...
     */
    public static Task2 findById(long id) {
        SqlSession ss = SessionFactory.getSqlSession();
        try {
            TaskMapper2 mapper = ss.getMapper(TaskMapper2.class);
            Task2 task2 = mapper.selectById(id);
            if (task2 == null) {
                return task2;
            }
	...
	public static boolean isTaoBaoAndTmall(long taskId) {
		Task2 task2=Task2.findById(taskId);
		System.out.println(""+task2.toString());
		System.out.println(""+task2.toString());
		System.out.println(""+task2.toString());
		/*if((task2.platform==Platform2.WEIXIN)&&task2.type!=TaskType2.PENGYOUQUAN){
			return false;
		}*/
			return false;
	}
     ...
     */
    public static List<Task2> findlockedPledgeTask(long sellerId) {
        SqlSession ss = SessionFactory.getSqlSession();
        try {
            TaskMapper2 mapper = ss.getMapper(TaskMapper2.class);
     ...
     */
    public static Task2 findById(String fields, long id) {
        SqlSession ss = SessionFactory.getSqlSession();
        try {
            TaskMapper2 mapper = ss.getMapper(TaskMapper2.class);
	 ...
	 */
	@NotNull
	public static Page<Task2> findBySearchVo(TaskSearchVo2 vo) {
		...
		try {
			...
			Page<Task2> page = Page.newInstance(vo.pageNo, vo.pageSize, count);
			List<Task2> tasks = mapper.selectList(vo);
	 ...
	 */
	@NotNull
	public static Page<SellerTaskVo2> findVoByPage(TaskSearchVo2 svo) {
		SqlSession ss = SessionFactory.getSqlSession();
		try {
			TaskMapper2 mapper = ss.getMapper(TaskMapper2.class);
			...
			Page<SellerTaskVo2> page = Page.newInstance(svo.pageNo, svo.pageSize, totalCount);
			List<SellerTaskVo2> vos =  mapper.selectVoListForSeller(svo);
			
			// 若获取条件有状态审核不通过，给不通过原因设值
            for (SellerTaskVo2 vo : vos) {
                ...
                if (svo.status == TaskStatus.NOT_PASS && MixHelper.isNotEmpty(vo.examineLogs)) {
                    vo.examineLog = SellerTaskVo2.getLastLog(vo);
                }
                // 返回任务状态的标题文字到前端
	 ...
	 */
	@NotNull
	public static Page<SellerTaskVo2> findProcessTaskByPage(TaskSearchVo2 svo) {
		SqlSession ss = SessionFactory.getSqlSession();
		try {
			TaskMapper2 mapper = ss.getMapper(TaskMapper2.class);
			int totalCount = mapper.countProcessTaskSeller(svo);
			System.out.println("（卖家）"+totalCount);
			System.out.println("（卖家）"+totalCount);
			System.out.println("（卖家）"+totalCount);
			System.out.println("（卖家）"+totalCount);
			System.out.println("（卖家）"+totalCount);
			...
			Page<SellerTaskVo2> page = Page.newInstance(svo.pageNo, svo.pageSize, totalCount);
			List<SellerTaskVo2> vos =  mapper.selectProcessTaskSeller(svo);
	 ...
	 */
	@NotNull
	public static Page<Task2> findByPage(TaskSearchVo2 vo) {
	    ...
	    try {
	        ...
	        Page<Task2> page = Page.newInstance(vo.pageNo, vo.pageSize, totalCount);
	 ...
	 */
	public static Page<SellerTaskVo2> findExamineVoByPage(TaskSearchVo2 vo) {
		SqlSession ss = SessionFactory.getSqlSession();
		try {
			TaskMapper2 mapper = ss.getMapper(TaskMapper2.class);
			...
			Page<SellerTaskVo2> page = Page.newInstance(vo.pageNo, vo.pageSize, totalCount);
			page.items = mapper.selectVoForStatus(vo);
			for (SellerTaskVo2 v : page.items) {
     ...
     */
    public static Multimap<String, TaskCountVo2> findWaitingTaskCountInfo(long sellerId) {
        List<TaskCountVo2> list = Lists.newArrayList();
        SqlSession ss = SessionFactory.getSqlSession();
        try {
            TaskMapper2 mapper = ss.getMapper(TaskMapper2.class);
            list.addAll(mapper.countByExamineStatusAndSellerId(sellerId));

            BuyerTaskMapper2 btMapper = ss.getMapper(BuyerTaskMapper2.class);
            list.addAll(btMapper.countWaitingTasksBySellerId(sellerId));
        } finally {
            ss.close();
        }

        Multimap<String, TaskCountVo2> map = ArrayListMultimap.create();
     ...
     */
    public static Task2 findTodoTask(long sellerId) {
        SqlSession ss = SessionFactory.getSqlSession();
        try {
            TaskMapper2 mapper = ss.getMapper(TaskMapper2.class);
     ...
     */
	public static long computeFeePerOrder(long itemPrice,Platform2 platform2) {
        // 转换金额单位：从分到元
        BigDecimal decimal = BigDecimal.valueOf(itemPrice).movePointLeft(2);
        // 以浮点数计算
        float result = computeCommission(decimal.floatValue(),platform2);
        // 转换金额单位：从元到分
     ...
     */
    public static float computeCommission(float price,Platform2 platform2) {
    	/*int taskStartIngot=BizConstants.TASK_STARTING_INGOT;
		if (platform2 != Platform2.WEIXIN) {
     ...
     */
    public long calculateRewardIngot(Device device) {
        ...
        // 移动端任务要再加上额外的金币
        return (long) (pcReward + BizConstants.TASK_MOBILE_INGOT2 * 100) + extReward;
    }
	...
	public void examine(String memo) {
		...
		try {
			...
			log.save();
			//更新流量记录
			TaskItemSearchPlanMapper searchPlanMapper=ss.getMapper(TaskItemSearchPlanMapper.class);
			List<TaskItemSearchPlan> searchPlan=searchPlanMapper.getOneTaskPlan(this.id);
			Task2 task = Task2.findById(this.id);
			if(CollectionUtils.isNotEmpty(searchPlan)){
				long totalFlow=0;
				for (TaskItemSearchPlan plan : searchPlan) {
					totalFlow+=plan.flowNum;
				}
				//任务审核部通过退回发任务花费的流量
			...
			if (this.status == TaskStatus.PUBLISHED || this.status == TaskStatus.WAIT_PUBLISH) {
				...
				if (CollectionUtils.isNotEmpty(searchPlan)) {
					...
					if (task.platform == Platform2.WEIXIN || task.platform == Platform2.QQ || task.platform == Platform2.WEIBO)
						for (TaskItemSearchPlan plan : searchPlan) {
							...
							if (plan.flowNum > 0) {
								...
								TrafficRecordVo trafficRecord = TrafficRecordVo.init2(task, plan.flowNum, plan.word);
            ...
            if (this.status == TaskStatus.PUBLISHED) {
                Task2.updateTaskCount(Sign.PLUS, task.totalOrderNum);
            }
		} finally {
			ss.close();
		}
    ...
    public static void cancledAndRefund(long id) {
        // 获取任务退款金额
        CancelTaskVo2 cancledInfo = findCancelInfo(id);
        // 若任务全部被接单
        
        Task2 t = Task2.findById(id);
        ...
        try {
            if (cancledInfo != null) {
                ...
                log.info("Cancel task={} UNLOCK pledge={}", id);
                ...
                // 更新“任务列表(X)”中的任务数量
                Task2.updateTaskCount(Sign.MINUS, cancledInfo.cancledNum);
                // 更新user缓存
                User.findByIdWichCache(seller.id).ingot(ingotRecord.balance).updateCache();
            }
            // 更新任务状态为取消
            Task2 ta = Task2.instance(id).setSellerId(seller.id).setStatus(TaskStatus.CANCLED).modifyTime(now);
            ss.getMapper(TaskMapper2.class).updateById(ta);
            ss.commit();
        } catch (Exception e) {
            ss.rollback();
            log.error(e.getMessage(), e);
	 ...
	 */
    public static CancelTaskVo2 findCancelInfo(long id) {
		SqlSession ss = SessionFactory.getSqlSession();
		try {
			// 取出已接单数量
			TaskMapper2 mapper = ss.getMapper(TaskMapper2.class);
            Task2 t = mapper.selectById(id);
            ...
            if (t.pcTakenCount <= 0 && t.mobileTakenCount <= 0) {
                CancelTaskVo2 vo =  CancelTaskVo2.newInstance(t.totalOrderNum, t.totalIngot);
    ...
    public boolean isTakenOver(Device device) {
        ...
        TaskSearchVo svo = TaskSearchVo.newInstance().taskId(this.id).statusExclude(TaskStatus.CANCLED);
        SqlSession ss = SessionFactory.getSqlSession();
        try {
	 ...
	 */
	   ...
	   */
	  public static List<Task2> findSellerPublishCount(String publishTime) {
          SqlSession ss = SessionFactory.getSqlSession();
          try {
              TaskMapper2 mapper = ss.getMapper(TaskMapper2.class);
	 ...
	 */
	public static Task2 findByItemId(String itemId) {
		SqlSession ss = SessionFactory.getSqlSession();
		try {
			TaskMapper2 mapper = ss.getMapper(TaskMapper2.class);
	 ...
	 */
	   public static void updateTaskExpressType() {
	        ...
	        try {
	            TaskMapper2 mapper = ss.getMapper(TaskMapper2.class);
	            List<Task2> tasks=mapper.findTaskExpressType();
    ...
    public static Task2 newInstance() {
        Task2 task = new Task2();
        return task;
    }
    
    public static Task2 instance(long id) {
        Task2 task = new Task2();
        task.id = id;
        return task;
    }
    
    public static Task2 instanceForTest(long id) {
        Task2 task = instance(id);
        task.pcOrderNum = 0;
	...
	public Task2 setSellerId(long sellerId) {
	    this.sellerId = sellerId;
	    return this;
	}
	
    public Task2 setStatus(TaskStatus status) {
        this.status = status;
        return this;
    }

    public Task2 setPublishTimerInterval(int publishTimerInterval) {
        this.publishTimerInterval = publishTimerInterval;
        return this;
    }

    public Task2 setLastBatchPublishTime(Date lastBatchPublishTime) {
        this.lastBatchPublishTime = lastBatchPublishTime;
        return this;
    }
    
    public Task2 speedTaskIngot(int speedTaskIngot) {
        this.speedTaskIngot = speedTaskIngot;
        return this;
    }
    
    public Task2 extraRewardIngot(int extraRewardIngot) {
        this.extraRewardIngot = extraRewardIngot;
        return this;
    }

    public Task2 modifyTime(Date modifyTime){
        this.modifyTime = modifyTime;
        return this;
    }
    
    public Task2 shopId(long shopId){
        this.shopId = shopId;
        return this;
    }
    
    public Task2 shopName(String shopName) {
        this.shopName = shopName;
        return this;
    }

    public Task2 sellerNick(String sellerNick) {
        this.sellerNick = sellerNick;
        return this;
    }
    
    public Task2 platform(Platform2 p) {
        this.platform = p;
        return this;
    }
    
    public Task2 orderNum(Device d, int orderNum) {
        Validate.notNull(d);
        if (d == Device.PC) {
    ...
    public Task2 itemUrl(String itemUrl) {
        this.itemUrl = itemUrl;
        return this;
    }
    
    public Task2 status(TaskStatus status) {
        this.status = status;
        return this;
    }
	@NotNull
		public static List<Task2> findALLList(TaskSearchVo2 vo) {
			...
			try {
				TaskMapper2 mapper = ss.getMapper(TaskMapper2.class);
				List<Task2> tasks = mapper.selectAllList(vo);
				return tasks;
