...
public class BuyerAccount {
     ...
     */
    public static List<BuyerAccount> findList(BuyerAccountSearchVo vo) {
        SqlSession ss = SessionFactory.getSqlSession();
        try{
        	BuyerAccountMapper mapper = ss.getMapper(BuyerAccountMapper.class);
        	BuyerTaskMapper taskMapper = ss.getMapper(BuyerTaskMapper.class);
        	List<BuyerAccount> list = mapper.selectByUserIdAndPlatform(vo.userId, vo.platform);
        	
        	BuyerTaskSearchVo taskVo = new BuyerTaskSearchVo();
        	taskVo.buyerId = vo.userId;
        	for(BuyerAccount ba: list){
     ...
     */
    public static List<BuyerAccount>  findForTakingTask(BuyerAccountSearchVo vo) {
        SqlSession ss = SessionFactory.getSqlSession();
        try{
            BuyerAccountMapper mapper = ss.getMapper(BuyerAccountMapper.class);
	 ...
	 */
	public static Page<BuyerAccount> findForPage(BuyerAccountSearchVo vo) {
		SqlSession ss = SessionFactory.getSqlSession();
		try {
			BuyerAccountMapper baMapper = ss.getMapper(BuyerAccountMapper.class);
			int count = baMapper.count(vo);
			if (count <= 0) {
				return Page.EMPTY;
			}
			Page<BuyerAccount> page = Page.newInstance(vo.pageNo, vo.pageSize, count);
			page.items = baMapper.selectBySearch(vo);
			for (BuyerAccount ba : page.items) {
				if(StringUtils.isNotBlank(ba.memo)){
        	        ba.memos = JsonUtil.toList(ba.memo, Memo.class);
        	    }
    ...
    public void updateStatus() {
        ...
        try {
            ...
            // 保存“审核不通过”时的理由
            if (this.status == ExamineStatus.NOT_PASS) {
                ...
                // 取出原来memo信息
                BuyerAccount bacount = mapper.selectById(id);
     ...
     */
    public static BuyerAccount findById(Long id){
       	SqlSession ss = SessionFactory.getSqlSession();
        try{
        	BuyerAccountMapper mapper = ss.getMapper(BuyerAccountMapper.class);
     ...
     */
    public static boolean validate(BuyerAccount account,User loginUser) {
        if (account == null) {
            return false;
        }
        return account.status == ExamineStatus.EXAMINED && account.belongTo(loginUser);
    }
