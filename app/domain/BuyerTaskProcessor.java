 ...
 */
public class BuyerTaskProcessor {
	 ...
	 */
	public synchronized static void confirmSysRefund(BuyerTask buyerTask, boolean isReject) {
		Validate.notNull(buyerTask.id);
		Validate.notNull(buyerTask.buyerId);
		BuyerTask bt = BuyerTask.instance(buyerTask.id).status(TaskStatus.WAIT_SYS_REFUND);
		if (isReject) {
		...
		try {
			...
			if (bt.status == TaskStatus.BUYER_REJECT_SYS_REFUND) {
				BuyerTaskStep sellerconfirmStep = stepMapper.selectByTaskIdAndTypeAndBuyerId(bt.id, BuyerTaskStepType.REFUND, buyerTask.buyerId);
				// 更新显示标记及修改时间
				if (sellerconfirmStep != null && BooleanUtils.isTrue(sellerconfirmStep.isValid)) {
					stepMapper.updateById(BuyerTaskStep.instance(sellerconfirmStep.id).valid(false).modifyTime(now));
				}
				return;
			}
			 ...
			 */
			BuyerTaskStep confirmStep = stepMapper.selectByTaskIdAndTypeAndBuyerId(bt.id, BuyerTaskStepType.CONFIRM_REFUND, buyerTask.buyerId);

			// 若之前未进行过改操作则新建一个进度记录
			if (confirmStep == null) {
				BuyerTaskStep taskStep = BuyerTaskStep.newInstance(bt.id, buyerTask.buyerId).type(BuyerTaskStepType.CONFIRM_REFUND).createTime(now)
						.modifyTime(now);
	 ...
	 */
	public synchronized static void cancelConfirmedSysRefund(BuyerTask bt) {
		...
		try {
			...
			// 将之前操作的任务进度标记置为“无效”
			BuyerTaskStepMapper stepMapper = ss.getMapper(BuyerTaskStepMapper.class);
			BuyerTaskStep confirmStep = stepMapper.selectByTaskIdAndTypeAndBuyerId(bt.id, BuyerTaskStepType.CONFIRM_REFUND, bt.buyerId);
			if (BooleanUtils.isTrue(confirmStep.isValid)) {
				stepMapper.updateById(BuyerTaskStep.instance(confirmStep.id).valid(false).modifyTime(now));
			}
	...
	public static void refundCancledBuyerTask(SqlSession ss, Task task, BuyerTask bt) {
	 ...
	 */
	public static void returnTaskIngot(BuyerTask bt) {
		Date now = DateTime.now().toDate();
