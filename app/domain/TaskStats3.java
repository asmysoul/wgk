...
public class TaskStats3 {
     ...
     */
    public static TaskStats3 findForBuyerUntilNow(long buyerAccountId) {
        
        String key = CacheType.BUYER_TASK_STATS.getKey(buyerAccountId);
        TaskStats3 stats = CacheUtil.get(key, TaskStats3.class);
        if (stats != null) {
            return stats;
        }
     ...
     */
    public static BuyerTask3 findLastTakenRecord(long buyerId,long shopId) {
        SqlSession ss = SessionFactory.getSqlSession();
        try {
            BuyerTaskMapper3 mapper = ss.getMapper(BuyerTaskMapper3.class);
     ...
     */
    public static List<BuyerTaskVo3> findBuyerIdAndSellerId(long buyerId,long sellerId){
     ...
     * @created 2015年1月13日 下午3:30:58
     */
    public static List<BuyerTaskVo3> findBuyerIdAndShopId(long buyerId,long shopId){
        Validate.notNull(buyerId);
     ...
     */
     public static List<BuyerTaskVo3> findBuyerAccountIdAndShopId(Long buyerAccountId, Long shopId) {
         ...
         try {
             ...
             DateTime end = DateTime.now();
             //从缓存中查找用户信息（关联商家配置信息），如果接任务配置存在，则使用商家配置的信息判断是否可接任务
             User seller = User.findByIdWichCache(Shop3.selectById(shopId).sellerId);
             String days = "";
             int day = 0;
     ...
     public static List<BuyerTaskVo3> findBuyerAccountIdAndShopId2(Long buyerAccountId, Long shopId) {
         Validate.notNull(buyerAccountId);
         Validate.notNull(shopId);
     ...
     * @param buyerAcountId
     */
    public static List<BuyerTaskVo3> selectByBuyerAccountIdAndItemIdAndPeriod(long buyerAcountId, String itemId) {
    	 SqlSession ss = SessionFactory.getSqlSession();
         try {
             BuyerTaskMapper3 mapper = ss.getMapper(BuyerTaskMapper3.class);
             DateTime end = DateTime.now();
             ...
             System.out.println("---------------------------------");
             //从缓存中查找用户信息（关联商家配置信息），如果接任务配置存在，则使用商家配置的信息判断是否可接任务
             User seller = User.findByIdWichCache(Task3.findByItemId(itemId).sellerId);
             String days = "";
             int day = 0;
    ...
    public static List<BuyerTask3> findBuyerTasksByItemId(long buyerId, String itemId) {
        SqlSession ss = SessionFactory.getSqlSession();
        try {
            BuyerTaskMapper3 mapper = ss.getMapper(BuyerTaskMapper3.class);
