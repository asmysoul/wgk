 ...
 */
public class BuyerTaskProcessor3 {
	 ...
	 */
	public synchronized static void confirmSysRefund(BuyerTask3 buyerTask, boolean isReject) {
		Validate.notNull(buyerTask.id);
		Validate.notNull(buyerTask.buyerId);
		BuyerTask3 bt = BuyerTask3.instance(buyerTask.id).status(TaskStatus.WAIT_SYS_REFUND);
		if (isReject) {
		...
		try {
			...
			if (bt.status == TaskStatus.BUYER_REJECT_SYS_REFUND) {
				BuyerTaskStep3 sellerconfirmStep = stepMapper.selectByTaskIdAndTypeAndBuyerId(bt.id, BuyerTaskStepType.REFUND, buyerTask.buyerId);
				// 更新显示标记及修改时间
				if (sellerconfirmStep != null && BooleanUtils.isTrue(sellerconfirmStep.isValid)) {
					stepMapper.updateById(BuyerTaskStep3.instance(sellerconfirmStep.id).valid(false).modifyTime(now));
				}
				return;
			}
			 ...
			 */
			BuyerTaskStep3 confirmStep = stepMapper.selectByTaskIdAndTypeAndBuyerId(bt.id, BuyerTaskStepType.CONFIRM_REFUND, buyerTask.buyerId);

			// 若之前未进行过改操作则新建一个进度记录
			if (confirmStep == null) {
				BuyerTaskStep3 taskStep = BuyerTaskStep3.newInstance(bt.id, buyerTask.buyerId).type(BuyerTaskStepType.CONFIRM_REFUND).createTime(now)
						.modifyTime(now);
	 ...
	 */
	public synchronized static void cancelConfirmedSysRefund(BuyerTask3 bt) {
		...
		try {
			...
			// 将之前操作的任务进度标记置为“无效”
			BuyerTaskStepMapper3 stepMapper = ss.getMapper(BuyerTaskStepMapper3.class);
			BuyerTaskStep3 confirmStep = stepMapper.selectByTaskIdAndTypeAndBuyerId(bt.id, BuyerTaskStepType.CONFIRM_REFUND, bt.buyerId);
			if (BooleanUtils.isTrue(confirmStep.isValid)) {
				stepMapper.updateById(BuyerTaskStep3.instance(confirmStep.id).valid(false).modifyTime(now));
			}
	...
	public static void refundCancledBuyerTask(SqlSession ss, Task3 task, BuyerTask3 bt) {
	 ...
	 */
	public static void returnTaskIngot(BuyerTask3 bt) {
		Date now = DateTime.now().toDate();
