...
function modifyAccount() {
	...
	Tr.post('/buyer/account/add2', params, function(data) {
		if (data.code == 8001) {
			alert('最多只能添加2个买号！');
		}
		if (data.code == 800101) {
...
function modifyOrderNumber() {
	...
	Tr.post('/buyer/account/modifyOrderNumber2', {
		id:CDT.currentId,
...
function initBaseBind() {
	...
	$(document).on('click', '.modifyBtn', function() {
		...
		Tr.get('/buyer/account/taskStatus2', {
			'btVo.accountId': id
		}, function(data) {
			if (data.code != 200) return;
			if (data.results.hasTaskExecuting) {
				alert('您有推广未完成，无法修改');
				$(this).text('推广未完成 无法修改');
				return;
			}
