...
function cancelTask(id){
	Tr.get('/admin/buyerTask/cancel3/'+id,{
	    'id': id
	},function(data){
		if(data.code == 400){
			alert('该子任务已取消');
...
function initBaseBind() {
	...
	$(document).on('click','.updateExpress',function(){
		var id = $(this).attr('data-id');
		var expressNo=$(this).attr("data-expressNo");
		$("#updateBuyerTaskId").val(id);
		$("#oldExpressNo").val(expressNo);
		Tr.popup('addAdminWnd');
	});
	$("#btnAddAccount").click(function(){
		var id=$("#updateBuyerTaskId").val();
		var expressNo=$("#newExpressNo").val();
		Tr.get('/admin/buyerTask/updateExpress3',{
		    'id': id,
		    'expressNo':expressNo
		},function(data){
			if(data.code!=200)return;
