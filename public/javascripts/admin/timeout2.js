...
function initBase() {
	...
	$(document).on('click', '#btnConfirm', function() {
		if(CDT.type=='sllerGood') {
			Tr.post('/admin/seller/task/sendGood', {
			...
			Tr.post('/admin/buyerTask/processTimeOut', {
				id:CDT.currUid,
			...
			Tr.post('/admin/buyerTask/processTimeOutPlat', {
				id:CDT.currUid,
				isSubtraction:false
			}, function(data) {
				if(data.code !=200) return;
			...
			Tr.post('/admin/task/confirmRecvGoods', {
			...
			Tr.post('/admin/buyer/task/confirmRefund', {
				'bt.id': CDT.currUid,
				'bt.status': 'FINISHED',
			...
			Tr.post('/admin/buyer/task/sysRefund/confirm', {
				'id': CDT.currUid,
				'isReject': false
			}, function(data) {
				if (data.code != 200) {
...
function loadTask(pageNo) {
	...
	loadCount();
	Tr.get('/admin/buyerTask/listAllTimeout2', {
...
function loadCount() {
	var createTimeStart = ($.trim($('#createTimeStart').val())) ? $.trim($('#createTimeStart').val()) + ' 00:00:00' : '',
		createTimeEnd = ($.trim($('#createTimeEnd').val())) ? $.trim($('#createTimeEnd').val()) + ' 00:00:00' : '';
	Tr.get('/admin/buyerTask/listAllTimeoutCount2', {
		'vo.platform': $('#platform').val(),
		'vo.device': $('#device').val(),
		'vo.type': CDT.type,
		'vo.buyerNick': $('#buyerNick').val(),
