...
function addAccount() {
	var nick = $('#txtNick').val(),
		...
		gender = $('input[name="gender_new"]:checked').val();
	alert(gender);
		//address = $('#txtAddress').val(), mobile = $('#txtMobile').val();
	Tr.post('/buyer/account/add3', {
...
function modifyAccount() {
	var $that = $('.currentCell').eq(0),
		...
		gender = $('input[name="gender"]:checked').val() == $.trim($that.find('.zcGender').text()) ? '' : $('input[name="gender"]:checked').val(), 		 	
	...
	if (!nick && !gender && !state && !city && !region) {
		alert('您没有修改任何内容');
		return;
	}
	var params = {
	...
	Tr.post('/buyer/account/add3', params, function(data) {
		if (data.code == 8001) {
			alert('最多只能添加2个买号！');
		}
		if (data.code == 800101) {
...
function modifyOrderNumber() {
	...
	Tr.post('/buyer/account/modifyOrderNumber3', {
		id:CDT.currentId,
...
function initBaseBind() {
	...
	$(document).on('click', '.modifyBtn', function() {
		...
		Tr.get('/buyer/account/taskStatus3', {
			'btVo.accountId': id
		}, function(data) {
			if (data.code != 200) return;
		...
		//alert($that.find('.zcGender').text());
		$('input[name="gender"]').each(function(index, item) {
			if ($(item).val() == $that.find('.zcGender').text()) {
