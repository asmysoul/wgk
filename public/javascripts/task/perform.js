...
function taskDeadlineCountdown() {
	...
	//var intDiff = Math.floor((10) - ((nowTime - date) / 1000)); //倒计时总秒数量
	var clock=setInterval(function() {
		...
		if(intDiff<0){            //'id': App.currId
			Tr.post('/buyer/task/cancel', {
				...
				alert('任务超时，自动撤销');
				window.location.replace('/buyer/tasks');
			});
...
function initBaseBind() {
	...
	$(document).on('click', '.buttonSection .nextEnable', function() {
		...
		if (CDT.currStep == 2) {
			...
			Tr.get('/buyer/task/perform/checkItem', {
				itemUrl: $input.val()
			}, function(data) {
				if (data.code == 555) {
		...
		if (CDT.currStep == 3) {
			if (!$('#taskStep-3 .itemPicBox').find('img').attr('src')) {
				alert('没有主图');
				return;
			}
		...
		// 保存任务进度
		Tr.post('/buyer/task/saveStep', p, function(data) {
			if (data.code == 800101) {
...
$(function() {
	...
	if((App.platform=='TAOBAO'||App.platform=='TMALL'||App.platform=='JD')&&App.taskType!='JHS'){//淘宝和天猫的订单
	...
	setTargetStep(CDT.currStep);
	//为第五步渲染买号收货地址信息
	Tr.get('/buyer/task/buyerAccount', {
