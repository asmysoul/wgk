...
CDT.CALC_STEP_4.calc2 = function() {
	...
	var unit = parseFloat($spSumCost.text().replace(',',''), 10) + parseFloat(CDT.freight*CDT.totalOrderNum, 10);
...
function loadDianpu(platform) {
	Tr.get('/user/shop/' + platform, {}, function(data) {
...
function setTargetStep(step) {
	...
	if(index==1) {
		...
		Tr.get('/user/shop/detail/'+id, {}, function(data) {
			if(data.code!=200)return;
...
function commissionTable(price) {
	...
	return Math.ceil((price - 100) / 150) + CDT.baseOrderFee;
}
...
$(function() {
	...
	$('.buttonSection .next').click(function() {
		...
		if (CDT.currStep == 1) {
			...
			if(plat!="TAOBAO"&&plat!="TMALL"){
				CDT.baseOrderFee=4;//其他平台5块起步
			}
		    	...
		    	if ($('#ckSearchTaobao').hasClass('inputCheckBox-checked')) {
		    		...
		    		$(wordPlanBoxes).each(function(n) {
						...
						html += "<div class='panelLine sec'>"
								...
								+ "<span class='floatLeft ls'>单</span><input type='hidden' name='searchItemFlowNum' value='"+flow+"'/></div>";
			    	});
		...
		if (CDT.currStep == 4) {
			...
			Tr.post('/task/save', p, function(data) {
				if (data.code == 800101) {
					alert('待支付的订单不能进行修改！');
				}
				if (data.code == 8001) {
	...
	$('input[name="r_tade_num"]:radio').click(function() {
		...
		$('.spSumCost').number(CDT.commissionFee * count, 2);
	...
	$('#txtCustomOrderNum').keyup(function() {
		...
		var wirelessOrderNum=(commissionTable(CDT.orderItemPrice) + 1)*parseFloat($('#wirelessOrderNum').val(),10);
	...
	$('#pcOrderNum').keyup(function() {
		...
		$('.spSumCost').number(CDT.commissionFee * theNum + (CDT.commissionFee + 1) * (totalNum - theNum), 2);
	...
	$('#wirelessOrderNum').keyup(function() {
		...
		$('.spSumCost').number(CDT.commissionFee * (totalNum - theNum) + (CDT.commissionFee + 1) * theNum, 2);
	...
	$('#btnConfirmPayment').click(function() {
		...
		Tr.post('/task/publish/confirmPay', {
