...
function loadCount() {
	...
	if(window.location.href.indexOf("admin")>0) {
		var url1 = '/admin/task/count3';
		var url2 = '/admin/task/deviceCount3';
		var url3 = '/admin/task/taskTypeCount3';
	}else {
		var url1 = '/task/count3';
		var url2 = '/task/deviceCount3';
		var url3 = '/task/taskTypeCount3';
	}
	Tr.get(url1, {}
			, function(data) {
		if (data.code != 200) 
	...
	Tr.get(url3, {platform : platform, device : device}, 
		function(data) {
		if(data.code !=200) return;
			$('#ORDER').text("自然搜索("+data.results.ORDER+")");
			$('#JHS').text("淘口令("+data.results.JHS+")");
...
function loadBuyerAccount() {
	...
	if(userType=='BUYER'){
	Tr.get('/buyer/account3/' + platform, {receive:true
	}, function(data) {
		if (data.code != 200) return;
...
function initBaseBind() {
	...
	$(document).on('click', '.btnSureTake', function() {
		...
		if(parseFloat($('#allingot').text(),10)>=1){
			...
			Tr.get('/buyer/task/taskDetil3', {
				taskId: $(this).attr('data-id')
				}, function(data){
					if(data.code!=200) return;
					$('#taskInfoShower .spTaskRequest').html("");
					$('#taskInfoShower .spTaskRequest').text(data.results);
//					var gender = $taskRow.find('.r_op > a').attr('gender-info');
//					var location = $taskRow.find('.r_op > a').attr('buyer-location-info');
//					if (gender != '') {
//						$('#taskInfoShower .spTaskRequest').append('<span>性别要求：'+gender+'</span>');
//						var uGender =  $('#buyerAccountPicker').find('a').attr('data-gender');
//						if (uGender != gender) {
//							newRulerDoTaskFLAG = false;
//						}
//					}
//					if (location != '') {
//						$('#taskInfoShower .spTaskRequest').append('<span>位置要求：'+location+'</span>');
//						var ulocation =   $('#buyerAccountPicker').find('a').attr('data-state');
//						ulocation = ulocation.replace('省', '');
//						if (location.indexOf(ulocation) <= -1) {
//							newRulerDoTaskFLAG = false;
//						}
//					}
			...
			$('#taskInfoShower .spAmount').text(str);
			$('#taskInfoShower .spCommission').text($taskRow.find('.rewardIngotSum').attr('rew'));
			var pc=$taskRow.find('.r_pc').text();
			var mobile=$taskRow.find('.r_mobile').text();
			if(pc>0){
				$('#taskInfoShower .spExpr').text("电脑端");
			}
			if(mobile>0){
				$('#taskInfoShower .spExpr').text("手机端");
			}
		}else{
	...
	$('#btnConfirmTake').click(function() {
		...
		Tr.post('/task/take3', p, function(data) {
			if (data.code == 8011 && confirm('上次接手的任务尚未完成，马上去做？')) {
				window.location.replace('/buyer/task/perform3/' + data.msg);
			}
			...
			if (data.code != 200) return;
				window.location.href = '/buyer/task/perform3/' + data.results;
...
function loadPage(pageNo) {
	...
	var url = nowUrl.indexOf("admin")>0?'/admin/task/list3':'/task/list3';
	var platform = $('#platform .selectedCtb').attr('value');
...
//					return this.extraRewardIngot == null ? 0:$.number(this.extraRewardIngot/100+imgIngot,2);
//				}
//				if(!this.extraRewardIngot){
//					if(device == 'PC'){
//						return this.baseOrderIngot == null ? 0:$.number(this.baseOrderIngot/100+imgIngot, 2);
//					}
//					if(device == 'MOBILE'){
//						return this.baseOrderIngot == null ? 0:$.number(this.baseOrderIngot/100+imgIngot, 2);
//					}
...
//				}
				if(device == 'PC'){
					//alert(this.totalIngot/100);
					//return $.number((parseInt(this.baseOrderIngot)+ parseInt(this.extraRewardIngot)+2),2);
					//alert("this.totalIngot====="+this.totalIngot);
					//alert("this.vasIngot====="+this.vasIngot);
					return $.number(((this.totalIngot/100)-0.2)/this.totalOrderNum,2);
					//return $.number(((parseFloat(this.totalIngot)/100-0.2)/parseFloat(this.totalOrderNum)),2);
				}
				if(device == 'MOBILE'){
					return $.number(((this.totalIngot/100)-0.2)/this.totalOrderNum,2);
					//return $.number((parseInt(this.baseOrderIngot)+ parseInt(this.extraRewardIngot)+2),2);
					//return $.number((parseFloat(this.baseOrderIngot)+ parseFloat(this.extraRewardIngot)),2);
				}
			},
//			extraRewardIngotFormat: function(){
//				// 80%加赏佣金
//				return $.number(this.extraRewardIngot, 1);
//			},
			baseOrderIngotFormat: function(){
				if(device == 'PC'){
					return $.number(this.baseOrderIngot,2);
				}
				if(device == 'MOBILE'){
					return $.number(this.baseOrderIngot,2);
				}	
