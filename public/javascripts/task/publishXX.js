...
CDT.CALC_STEP_4.calc2 = function() {
	...
	var unit = parseFloat($spSumCost.text().replace(',',''), 10)+ 2*CDT.totalOrderNum;
...
//		}
    ...
    function initGenderAndLocationConfig() {
    	if ($("input:radio[name='gender']:checked").length > 0) {
    		$('.totalGenderIngot').html(parseInt($('#genderIngot').html()) * CDT.totalOrderNum);	
    	} else {
    		$('.totalGenderIngot').html("0");
    	}
    	
    	if($('.p_active').length > 0) {
    		$('.totalBuyerLocationIngot').html(parseInt($('#buyerLocationIngot').html()) * CDT.totalOrderNum);	
    	} else {
    		$('.totalBuyerLocationIngot').html("0");
    	}
    	CDT.CALC_STEP_4.calc1();
...
function getGenderAndLocationConfigTotal() {
	...
	var c = 0;
	if ($("input:radio[name='gender']:checked").length > 0) {
		g = parseInt($('#genderIngot').html()) * CDT.totalOrderNum;	
	} 
	
	if($('.p_active').length > 0) {
		b = parseInt($('#buyerLocationIngot').html()) * CDT.totalOrderNum;	
	} 
	if ($("input:radio[name='collectionType']:checked").length > 0) {
		c=parseInt($('#collectionIngot').html()) * CDT.totalOrderNum;	
	}
	return g + b+c;
}
